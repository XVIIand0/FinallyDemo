...
<%
	...
	sql="SELECT * FROM good;";
                            ...
                            int l=current_gid;
                            int r=current_gid;
							sql="SELECT AVG(m_g_star) as stars FROM `order` where product="+current_gid+";";
								ResultSet rs2 = con.createStatement().executeQuery(sql);
								rs2.next();
								String stars ="1";
								String tell="";
								if(rs2.getString("stars") == null){
								tell ="暫無評分!";
								stars = "0";
								   
								}
								else{
									stars = rs2.getString("stars");
								}
...
.rating:after {
  content: "";
  <%out.print("width: "+( Double.valueOf(stars).doubleValue()*20)+"%;");%>
  height: 30px;
  position: absolute;
  top: 0;
  left: 0;
...
<%
								...
								if(tell=="暫無評分!"){
								out.println("<h5>尚無商品評價~</h5>");
								}
								else{
								out.print("<style></style>");
								out.print("<div class='star-rating'>");
                                out.println("<h5><div class='rating rating-1'><img src='https://s.cdpn.io/2437/mushroom.png' alt='Mushroom!' /></div></h5><br></div>");
								}
